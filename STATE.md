...
**Opus verification (conf 0.9): the SQL lane was the strongest of the project** — 700/700 re-run, 3/3 function file↔live byte-match, and the privacy ruling held against SIXTEEN distinct attacks from a genuinely paired peer. What blocked was the gate's own instruments: **(F18) the detekt println ban was provably INERT** (@RequiresTypeResolution with no classpath — 167/167 files scanned, two live printlns, zero findings; the second vacuous-rail incident) → replaced by a root `assertNoPrintln` Gradle task wired into `check` + CI, and **proven to fire** (a probe file failed the build, then removed). **(F21) the evidence package failed again** — one byte-identical duplicate (deleted), one shot asserting a Save the ledger disproved (the pre-DI-fix build; STATE's "works" claim was unsupported at the time), and the "minified sign-in" shot showing only a spinner. Corrections: the ran-long Save is NOW proven with ledger evidence — **`ran_long / 75 / version 3`** after a fresh sign-in + tap flow on the fixed build (`stage5-e2e-05-ranlong-saved.png`, all seven shots distinct md5s); the minified sign-in leg stands as lead-attested only (shot 06 is the loading screen; shot 07's Today-under-R8 remains the artifact). **(F11)** `RsvpLinkConfig` now comes from BuildConfig with the release assertion covering BOTH endpoints, https AND non-loopback. **(F12)** `docs/dependency-audit.md` written (incl. the FLAG_SECURE declined-and-recorded ruling), CI gained the release-shape job (unsigned bundle, manifest greps, exported-pin, println rail, mapping archive).

**Non-blocking fixes:** btrim'd title matching + `so.id` tiebreak in the suggestion RPC (700/700 after); keystore.properties present-but-incomplete now fails with a named message; release.md documents the escape hatch + the real smoke procedure; the three remaining log sites are class-name-only; stale D2 NOTE removed. **Deferred by contract amendment 5:** the outcome read RPC + foreground retry (F24/F25 — re-prompting is safe-but-nagging until then). The AAB sha256 is `7b7497aaad4298542692240099248bcc173e1846318d61a3819e18636d1b29cb` (9,800,064 B, versionCode 10000, an `-Pelay.allowInsecureRelease` local-smoke build — never distribute). Master-plan gaps the audit surfaced (deep links; onboarding moment) are recorded in the closure entry as explicitly unshipped scope.
...
**Closure checklist:**
- [x] Contract + 7 amendments implemented; migration `20260912200000` (890 lines) + pgTAP **616/616** (22 suites; 0022 now 97 assertions incl. the F12 manual-sync-rejection and F8 purge pins added at re-verify).
- [x] Live E2E: manual busy → self-hints `busy`; on device the responder chip reads the honest "Free · matches your busy times" and the composer's own badge "You have a scheduled block at this time" (three distinct captures, `stage4-e2e-01..03`).
- [x] Verification: Opus two-round pass (round 1 conf 0.88, 7 blockers — ALL honesty/data-loss class; round 2 **flips to ready, conf 0.9**, both new SQL guards repro-verified, file↔live 15/15 byte-match). Gemini's parallel pre-gate seat flaked (0 chars) — closure rests on the Opus double-pass + its rolled-back repros. Ledger corrections applied for real this round: the F7 comments (my earlier "fixed" claim was false — the sed hadn't matched), the line count (890; 870/881 both stale).
- [x] CI ×3 green on `5e10be3`; the follow-up (pins + comments) rides the closure commit.

**Carried to the availability-surfacing pass (verifier's list):** partner-hints consumption + staleness copy (amendment 6); F6 starvation re-test when partner hints ship; F13 status-aware ladder; F11 fn_sync moved-event drop (MUST fix before the Google adapter); F9 domain error for own-synced-row edits; refreshCardHints debounce.
...
**All three Stage 4 seats merged same-session** (A6 SQL 890 lines (post-fix count) + 94 pgTAP; B7 client data 21 tests; C6 UI — certainty badges, manual "+ Busy" sheet, capacity gauge — 311 host tests total). **pgTAP 613/613** (22 suites). Lead work at merge: 0015's hints key-set pin gains `certainty`; B7's params reconciled to A6's real signatures (`p_id`, optional `p_label` — the label is owner-only, never in peer projections); **true-upsert fix** (A6 treated a client-generated id as update-only → P0002; house convention inserts — T-CLIENTTAG-3 now pins the synced-row guard via the pk collision); DI wiring (AvailabilityRepository into UserGraph/App.kt + the DI test's noop).
...
**Seats B5 (client mint, 263 tests) and C4 (share UI, 266 tests) merged — all four Stage 3 seats now in.** The signature end-to-end, all live (shots `stage3-e2e-01..03.png` + committed page captures `stage3-e2e-04..06`): A creates a proposal (curl-as-A; app shows the outgoing "Sent · Waiting for b2" card with dual-time + deadline) → taps **"Share response link"** → sheet renders the link, "Link works until Tomorrow at 4:00 PM", and the §A disclosure line built from the server's own `discloses` list → that exact app-minted link opened as the recipient: page says "Responding as b2 · Web share E2E" → **no-JS form accept → "Time lock confirmed · On both plans"** (the accept page itself is committed as `stage3-e2e-04-accept-page.html`; live headers in `-06`) → server: exactly 2 shared_lock blocks, token `consumed_action=accept` → **A's app card flips to TIME LOCK CONFIRMED via the realtime event** (before/after shots; the share sheet was opened between them, but the mint path provably kicks no refetch — verifier-confirmed — so the flip is realtime by construction). The product's week-one hook now works without the second device even having the app open.
