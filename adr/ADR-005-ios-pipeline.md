...
**Decision.** `.github/workflows/ios-verify.yml` on `macos-15`: `./gradlew :shared:iosSimulatorArm64Test` + framework **link** (not just compile: `linkDebugFrameworkIosSimulatorArm64`) + an unsigned `xcodebuild` simulator build (`CODE_SIGNING_ALLOWED=NO`) — required on every push/PR **from Phase 0**; **simulator launch + Room create/write/reopen smoke** (Astra 2026-09-11: compilation alone proves neither linking, embedding, DB startup nor auth callbacks) joins the job with the first Room migration in Phase 1, and no Phase 1 gate closes without it. Database/pgTAP CI runs on a **Linux** runner; macOS is reserved for Apple builds. Agents drive it headlessly from Windows: `gh workflow run` / `gh run watch --exit-status` / `gh run view --log-failed`. Local Macs are for interactive diagnosis only, never the gate. Signing/App Store secrets: not applicable (app won't ship); if that changes, they live in an approval-protected GitHub environment.
