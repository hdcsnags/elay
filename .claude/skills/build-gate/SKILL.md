...
**`gradlew ... | tail` swallows the exit code** — a BUILD FAILED gate reported exit 0 and a stale APK got installed and E2E'd (2026-09-12). Always `set -o pipefail` (bash) before piping gradle output, and/or end with `&& echo GATE_GREEN` and require the marker. Never treat exit 0 of a piped gate as green without seeing the marker or the BUILD SUCCESSFUL line.
