...
- **Codex MODE=code is broken on this machine as of 2026-09-11**: workspace-write sandbox fails with `helper_unknown_error: setup refresh had errors` on any write (verified with a one-line write probe, `docs/audits/elay/2026-09-11-write-probe/`). Sol stays read-only (design/verification) until codex is updated/fixed — probe write mode with a one-liner before trusting it again.
- **Claude coder seats (Sonnet/Fable) do NOT work through council-dispatch MODE=code** (found 2026-09-11: the adapter's task mode is `--print` without write permissions — the seat stalls asking for access and returns an empty patch; the adapter's `runSession` path uses `--dangerously-skip-permissions`, and wiring that into the dispatch script is blocked by the concierge harness's safety classifier — see PING). **Working pattern instead:** the concierge makes a scratch clone (`git clone "C:\New folder\Elay" "C:\New folder\claw-scratch\elay-seat-<x>"` + copy `local.properties`), dispatches a Sonnet subagent via the harness Agent tool pointed at that clone with the brief file, then reviews `git -C <clone> diff` and applies. Same isolation, permissions managed by the concierge session.
